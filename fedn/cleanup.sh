...
echo "=== FEDn Cleanup ==="
echo "This will STOP server and client VMs."
read -p "Type STOP to continue: " confirm
[ "$confirm" = "STOP" ] || { echo "Cancelled"; exit 0; }

source vm-info.txt || true

if [ -n "${SERVER_VM:-}" ]; then
  echo "Stopping server VM $SERVER_VM"
  gcloud compute instances stop "$SERVER_VM" --zone="$SERVER_ZONE" --quiet || true
fi
for i in $(seq 1 5); do
  ...
  VM_ZONE=${!ZONE_VAR:-}
  [ -n "$VM_NAME" ] || continue
  echo "Stopping client VM $VM_NAME"
  gcloud compute instances stop "$VM_NAME" --zone="$VM_ZONE" --quiet || true
done
